...
namespace LT.Shi
{
    ...
    public class ThrowController : MonoBehaviour
    {
        ...
        private vThirdPersonController input;
        ...
        void Update()
        {
            ...
            if (aiming)
            {
                input.RotateToDirection(transform.position);
                transform.LookAt(Camera.main.transform.forward);
            }
            else
            {
                transform.eulerAngles = new Vector3(Mathf.LerpAngle(transform.eulerAngles.x, 0, .2f), 
                    transform.eulerAngles.y, transform.eulerAngles.z);
            }

            //Animation States
            animator.SetBool("Pulling", pulling);
            //walking = input.moveSpeed > 0;
            ...
            if(Input.GetKeyUp(KeyCode.Q) && hasWeapon && aiming)
            {
                Aim(false, true, 0);
            ...
            if (hasWeapon)
            {
                // if (aiming && Input.GetKeyUp(KeyCode.Q))
                // {
                //     animator.SetTrigger("Throw");
                // }
            }
            ...
            if (pulling)
            {
                if(returnTime < 1)
                {
                    weapon.position = GetQuadraticCurvePoint(returnTime, pullPosition, 
                        curvePoint.position, hand.position);
