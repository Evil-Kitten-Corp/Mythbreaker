...
namespace Quests.Utilities
{
    public class ScriptableObjectListPrinter 
    {
        ...
        private const float TextareaHeight = 50;
        private readonly Dictionary<string, bool> _textareaFields = new();

        private readonly HashSet<string> _variableBlacklist = new()
        {
            "m_Script",
            "_id",
            "_parent",
        ...
        public float GetHeight(int index) 
        {
            ...
            while (propIterator.NextVisible(true)) {
                ...
                if (RegisterTextareaAttributes(propIterator)) {
                    ...
                    totalHeight += TextareaHeight + labelHeight + EditorGUIUtility.standardVerticalSpacing;
                } else {
        ...
        private bool RegisterTextareaAttributes(SerializedProperty prop) 
        {
            ...
            if (_textareaFields.ContainsKey(propId))
            {
        ...
        {
            if (_textareaFields.TryGetValue(propId, out var currentValue) && currentValue) 
                return;
            
            var field = type.GetField(prop.name, BindingFlags.Instance | BindingFlags.NonPublic | 
                                                 BindingFlags.Public);
        ...
        {
            var labelHeight = EditorStyles.label.CalcHeight(new GUIContent(propIterator.displayName), 
                position.width);
            ...
            var textAreaPosition = new Rect(position)
            {
