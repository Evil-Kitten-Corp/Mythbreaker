...
namespace Stats
{
    ...
    {
        private static bool InRange(this Stat myStat, float min, float max)
        {
            return myStat.Current >= min && myStat.Current <= max;
        }

        private static bool IsPercentile(this Stat myStat)
        {
        ...
        public static void IncreasePercentile(Stat myStat, Stat stat)
        {
            ...
            {
                throw new ArgumentException($"Failed to increase stat value because received stat " +
                                            $"value is not between 0f and 1f");
            }
        ...
        public static void DecreasePercentile(this Stat myStat, Stat stat)
        {
            if (!IsPercentile(stat))
            {
                throw new ArgumentException($"Failed to decrease stat value because received stat " +
                                            $"value is not between 0f and 1f");
            }
