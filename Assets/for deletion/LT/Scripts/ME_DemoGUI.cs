...
public class ME_DemoGUI : MonoBehaviour
{
	...
	private void OnGUI()
	{
		...
		if (GUI.Button(new Rect(10f * dpiScale, 63f * dpiScale + offset, 285f * dpiScale, 37f * dpiScale), "Day / Night") || (!isButtonPressed && Input.GetKeyDown(KeyCode.DownArrow)))
		{
			...
			float lightInten = ((!UseMobileVersion) ? 1f : 0.3f);
		...
		GUI.Label(new Rect(400f * dpiScale, 15f * dpiScale + offset / 2f, 100f * dpiScale, 20f * dpiScale), "Prefab name is \"" + Prefabs[currentNomber].name + "\"  \r\nHold any mouse button that would move the camera", guiStyleHeader);
		GUI.DrawTexture(new Rect(12f * dpiScale, 140f * dpiScale + offset, 285f * dpiScale, 15f * dpiScale), HUETexture, ScaleMode.StretchToFill, false, 0f);
		float num = colorHUE;
		colorHUE = GUI.HorizontalSlider(new Rect(12f * dpiScale, 147f * dpiScale + offset, 285f * dpiScale, 15f * dpiScale), colorHUE, 0f, 360f);
		if ((double)Mathf.Abs(num - colorHUE) > 0.001)
		{
			PSMeshRendererUpdater meshUpdater = characterInstance.GetComponentInChildren<PSMeshRendererUpdater>();
			if (meshUpdater != null)
			{
